...
void addEdge(Graph graph, int v1, int v2, double w)
{
    ...
    {
      JRB tree1,tree2, node;
      node = jrb_find_int(graph.edges, v1);
      if (node == NULL)
        {
          tree1 = make_jrb();
          jrb_insert_int(tree1, v2, new_jval_d(w));
...
void setVertexParent(JRB tree,int v,int p){
  ...
  find=jrb_find_int(tree,v);
  if(find!=NULL){
    find->val=new_jval_i(p);
  }
  else jrb_insert_int(tree,v,new_jval_i(p));
}
...
double shortestPath(Graph graph, int s, int t, Dllist path, int *length)
{
    ...
    *length=0;
    jrb_traverse(ptr,graph.vertices){
      jrb_insert_int(d,jval_i(ptr->key),new_jval_d(INFINITIVE_VALUE));
    }
    ...
    while (!dll_empty(q))
    {
      ...
      dll_traverse(i, q)
        {
          if(getd(d,jval_i(i->val))<=min2) {
            min2=getd(d,jval_i(i->val));
            min=jval_i(i->val);
            save=i;
          }
        }
      if(min==t){
        ...
        do{
          ...
          *length++;
          p+=getEdgeValue(graph,getParent(parent,index),index);
          index=getParent(parent,index);
        }while(index!=s);
      ...
      jrb_traverse(ptr, graph.vertices)
        {
          if (hasEdge(graph, min, jval_i(ptr->key)))
            {
              u=jval_i(ptr->key);
              if (getd(d,u) > getd(d,min) + getEdgeValue(graph,min,u))
                {
                  setVertexd(d,u,getd(d,min) + getEdgeValue(graph,min,u));
                  setVertexParent(parent,u,min);
                }
              if (jrb_find_int(visited,u) == NULL && !isInDll(q,u))
